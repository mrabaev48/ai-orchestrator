...
const runtimeConfigSchema = z.strictObject({
  ...
  workflow: z.strictObject({
    ...
    fencingTtlMs: z.number().int().positive().optional(),
...
const envSchema = z.object({
  ...
  WORKFLOW_FENCING_TTL_MS: z.coerce.number().int().positive().default(60_000),
...
export function loadRuntimeConfig(options: LoadRuntimeConfigOptions = {}): RuntimeConfig {
  ...
  const merged = {
    ...
    workflow: {
      ...
      fencingTtlMs: env.data.WORKFLOW_FENCING_TTL_MS,
